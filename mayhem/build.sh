...
# Build-time LSan-off hook (SPEC.md 6.1, mayhem/lsan_off.c), linked into every libFuzzer target AND
...
# NOTE: no compiled-in ASan/LSan runtime-option override here. Mayhem alone owns the sanitizer
# runtime option set — a harness-baked override used to be sanctioned as a fix for LSan's ptrace
# conflict with Mayhem's own coverage-collection tracer, but real dispatched runs on other repos
# came back healthy (edges up, not zero) once the override was removed, so it is now forbidden.
...
build_one() {
  ...
  $CXX $CXXFLAGS $DEBUG_FLAGS $LIB_FUZZING_ENGINE "$SRC/mayhem-build/$name.o" "$LSAN_OBJ" \
      $LIBS $SYS -o "$OUT/$name"

  # standalone reproducer -> $OUT/<name>-standalone (no libFuzzer; one input file)
  $CXX $CXXFLAGS $DEBUG_FLAGS "$SRC/mayhem-build/$name.o" "$STANDALONE_OBJ" "$LSAN_OBJ" \
